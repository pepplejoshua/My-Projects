...
public:
    // Constructor
    Upper_Triangular_Matrix(int n, double value = 0.0);
    // Destructor
...
/* map(int, int) FUNCTION:
 for each 0 < i < n (rows)
 for each i < j < n, (columns), this restricts the number of columns to not exceed the number of rows because of the actual indexes being used to store values, creating the right angled triangle of numbers needed. The formular n ( n + 1 ) / 2 gives the number of necessary storage spaces.
